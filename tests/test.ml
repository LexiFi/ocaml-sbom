...
let test_capture_stdout =
  Testo.create "capture stdout" (fun () ->
      let buf = Buffer.create 10 in
      (match Sbom_util.Command.run ~output:(Buffer buf) [ "echo"; "hello" ] with
      | 0 -> ()
      | n -> ksprintf Testo.fail "command exited with code %i" n);
      Testo.(check string) "hello\n" (Buffer.contents buf))

let test_read_from_string =
  Testo.create "read from string" (fun () ->
      let buf = Buffer.create 10 in
      (match
         Sbom_util.Command.run ~input:(Data "the input") ~output:(Buffer buf)
           [ "cat" ]
       with
      | 0 -> ()
      | n -> ksprintf Testo.fail "command exited with code %i" n);
...
let test_missing_command =
  Testo.create "missing command" (fun () ->
...
let tests _env = command_tests @ Test_overlay.tests @ Test_heuristics.tests
