...
let rec license_expr_to_string (expr : S.license_expr) : string =
  ...
  | And (a, b) ->
      Printf.sprintf "(%s AND %s)"
        (license_expr_to_string a) (license_expr_to_string b)
  | Or (a, b) ->
...
let checksum_to_cdx (cs : S.checksum) : C.hash =
  C.create_hash ~alg:(checksum_alg cs.kind) ~content:(C.create_hashcontent cs.value) ()
...
let external_refs_of_component (c : S.component) : C.externalReference list =
  ...
  let src_ref =
    if sd.url = "" then None
      ...
      Some (C.create_externalReference
              ~url:(C.String sd.url)
              ~type_:C.Sourcedistribution
              ~hashes ())
  in
...
let actor_to_contact (actor : S.actor) : C.organizationalContact =
  ...
  | Organization o -> C.create_organizationalContact ~name:o.name ?email:o.email ()
...
let cdx_scope (s : S.dep_scope) : C.componentScope =
  match s with
  | Runtime | Build -> C.Required
  | Test | Dev | Optional -> C.Optional

let merge_scope (a : C.componentScope) (b : C.componentScope) : C.componentScope =
  match a, b with
  | C.Required, _ | _, C.Required -> C.Required
  | _ -> C.Optional

let build_scope_map (edges : S.dep_edge list) : (string, C.componentScope) Hashtbl.t =
  let tbl = Hashtbl.create 64 in
  List.iter (fun (e : S.dep_edge) ->
...
let component_to_cdx
    (scope_map : (string, C.componentScope) Hashtbl.t)
    (c : S.component) : C.component =
  let purl = (c.key :> string) in
  let licenses = licensing_to_cdx c.licensing in
  let ext_refs = match external_refs_of_component c with
    | [] -> None | refs -> Some refs in
  let authors = match c.authors @ c.maintainers with
    | [] -> None | cs -> Some (List.map actor_to_contact cs) in
  let hashes = match c.source_distribution.checksums with
    | [] -> None | cs -> Some (List.map checksum_to_cdx cs) in
  C.create_component
    ~type_:C.Library
    ~name:c.name
    ~version:(C.create_version c.version)
    ~bomref:(C.create_refType purl)
    ~purl
...
let of_document (doc : S.document) : C.cycloneDXBillofMaterialsStandard =
  ...
  C.create_cycloneDXBillofMaterialsStandard
    ~bomFormat:C.CycloneDX
    ~specVersion:"1.6"
