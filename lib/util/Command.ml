...
type input = Default | File of string | Data of string
type output = Default | File of string | Buffer of Buffer.t | Ignore
type capture = { status : int; output : string; error_output : string }
...
let setup_cmd_output ~default:default_fd (out : output) =
  match out with
  | Default -> (default_fd, (fun () -> ()), None)
  | File path ->
      let fd =
        Unix.openfile path [ Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC ] 0o666
      in
      (fd, (fun () -> Unix.close fd), None)
  | Buffer buf -> setup_optional_output_buffer (Some buf)
  | Ignore ->
        ...
        (fun task ->
          ...
          | Read (fd, opt_buf) when List.mem fd readable ->
              ...
              if n = 0 then (
                ...
                (match opt_buf with
                | Some buf -> Buffer.add_subbytes buf tmp 0 n
                | None ->
                    (* discard data *)
                    ());
                Some task)
...
let run ?(input : input = Default) ?(output : output = Default)
    ?(error_output : output = Default) ?name argv =
  match argv with
  ...
  | prog :: _ -> (
      let cmd_name =
        ...
        | None -> prog
      in
      let child_stdin, close_child_stdin, stdin_task = setup_cmd_input input in
      let child_stdout, close_child_stdout, stdout_task =
        setup_cmd_output ~default:Unix.stdout output
      in
      let child_stderr, close_child_stderr, stderr_task =
        setup_cmd_output ~default:Unix.stderr error_output
      in
      let argv_arr = Array.of_list argv in
      let pid =
        Unix.create_process cmd_name argv_arr child_stdin child_stdout
          child_stderr
      in
...
let run_capture ?input ?name argv =
  ...
  let status =
    run ?input ~output:(Buffer out_buf) ~error_output:(Buffer err_buf) ?name
      argv
  in
  {
    status;
